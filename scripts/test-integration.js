...
// Socket.IO client is a frontend dependency, so we'll make this test conditional
...
function log(message, color = 'reset') {
  // Sanitize message to prevent log injection (remove control characters)
  const sanitizedMessage = String(message).replace(/[\r\n\t\f\v\b]/g, ' ');
  console.log(`${colors[color]}${sanitizedMessage}${colors.reset}`);
}
...
async function testCORSConfiguration() {
  try {
    const response = await axios.options(`${API_BASE_URL}/status`, {
      headers: {
...
async function testSocketIOConnection() {
  // Skip Socket.IO test since it requires frontend dependencies
  logTest('Socket.IO Connection', 'SKIP', 'Frontend dependency not available in backend tests');
  return true;
}
